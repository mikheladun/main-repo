...
class Query
{
 	...
 	if($index > $this->ParamCount || $index < 1)
 	{
 		switch($type)
		{
			case $this->NUMBER:
				...
				break;
			case $this->STRING:
				$this->aParam[$index - 1] = ($param == NULL ? "NULL" : "'".FormatStringForDatabaseInput($param)."'");
				break;
			case $this->DATE:
				$this->aParam[$index - 1] = ($param == NULL || $param == 'SYSDATE' ? "NOW()" : "$param");
				break;
			default:
				$this->aParam[$index - 1] = FormatStringForDatabaseInput($param);
		}
	}
 }
 function Execute()
 ...
 {
 	ksort($this->aParam);
  	//echo "replaceArr[",print_r($this->replaceArr),"]<br/>";
  	//echo "patternArr[",print_r($this->patternArr),"]<br/>";
