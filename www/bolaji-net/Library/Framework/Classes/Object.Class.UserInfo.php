...
class UserInfo {
	...
	var $InstantMessengerClient;
	var $InstantMessengerScreenName;
	var $AboutMe;
	var $Birthday;
	var $Location;
	var $Website;
   ...
   var $SendNewApplicantNotifications;
	...
	function FormatPropertiesForDisplay() {
		...
		$this->InstantMessengerClient = FormatStringForDisplay($this->InstantMessengerClient, 0);
		$this->InstantMessengerScreenName = FormatStringForDisplay($this->InstantMessengerScreenName, 0);
		$this->AboutMe = FormatStringForDisplay($this->AboutMe, 0);
		$this->Birthday = FormatStringForDisplay($this->Birthday, 0);
		$this->Location = FormatStringForDisplay($this->Location, 0);
		$this->Website = FormatStringForDisplay($this->Website, 0);
	}
	...
	function GetPropertiesFromDataSet($DataSet) {
		...
		$this->InstantMessengerClient = ForceString(@$DataSet['INSTANTMESSENGERCLIENT'],'');
		$this->InstantMessengerScreenName = ForceString(@$DataSet['INSTANTMESSENGERSCREENNAME'],'');
		$this->AboutMe = ForceString(@$DataSet['ABOUTME'],'');
		$this->Birthday = FormatDate("M jS Y",UnixTimestamp(@$DataSet['BIRTHDAY']));
		$this->Location = ForceString(@$DataSet['LOCATION'],'');
		$this->Website = ForceString(@$DataSet['WEBSITE'],'');
		...
		$this->DateFirstVisit = FormatDate("M jS Y",UnixTimestamp(@$DataSet['DATEFIRSTVISIT']));
