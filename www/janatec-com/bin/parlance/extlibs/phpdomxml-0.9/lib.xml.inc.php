<?php
	...
	***************************************************************************/	
	...
	class XML_Element extends XML_Node {
		...
		function toHtmlString($pretty = false, $tabs = '') {
			$s = '';
			if ($pretty) $s .= $tabs;
			$s .= '<'.$this->nodeName;
			...
			if ($this->hasChildNodes()) {
				...
				if ($pretty) $s .= "\n";
				foreach ($this->childNodes as $child)
					$s .= $child->toHtmlString($pretty, $tabs."\t");
				if ($pretty) $s .= $tabs;
				$s .= '</'.$this->nodeName.'>';
				if ($pretty) $s .= "\n";
			} else {
		...
		function toString($pretty = false, $tabs = '') {
			...
			if ($this->hasChildNodes()) {
				$s .= '>';
				if ($pretty && count($this->childNodes)  > 1) {
					$s .= "\n";
				}
				foreach ($this->childNodes as $child) {
				...
				if($this->nodeName == 'textarea' || $this->nodeName == 'option') {
					$s .= '</' . $this->nodeName . '>';
				}
